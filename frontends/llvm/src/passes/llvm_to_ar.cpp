...
public:
  ...
  std::vector< std::string > get_sources_from_dest(std::string dest);
...
MDNode* find_debug_global_declare(GlobalVariable* gv) {
  Module* M = gv->getParent();
  NamedMDNode* NMD = M->getNamedMetadata("llvm.dbg.cu");
  if (!NMD)
    return NULL;

  for (NamedMDNode::op_iterator mdnode = NMD->op_begin();
       mdnode != NMD->op_end();
       mdnode++) {
    DICompileUnit* cu = dyn_cast< DICompileUnit >(*mdnode);

    DIGlobalVariableArray di_global_vars = cu->getGlobalVariables();
    for (unsigned int i = 0; i < di_global_vars.size(); i++) {
      DIGlobalVariable* digv = di_global_vars[i];
      if (digv->getVariable() == gv)
        return digv;
    }
  }
  return NULL;
...
Instruction* find_dbg_user(GlobalVariable* gv) {
  Value::use_iterator u = gv->use_begin();
  // Iterate over each use.
  for (; u != gv->use_end(); u++) {
    User* user = u->getUser();
    // In some cases, a user can be an argument of a call instructions.
    // For example, a std::string constant used as a getelementptr in a
    // printf().
    // In this case we iterate over the chain of uses and return its first
    // debuggable user.
    Value::use_iterator _u = user->use_begin();
    for (; _u != user->use_end(); _u++) {
      Instruction* inst = dyn_cast< Instruction >(*_u);
      if (inst && inst->getDebugLoc() && !inst->getDebugLoc()->isResolved()) {
        return inst;
      }
    }
  }
...
void GlobalReferences::set_debug_info(GlobalVariable* gv) {
  ...
  MDNode* node = find_debug_global_declare(gv);
  if (node) {
    DIGlobalVariable* digv = dyn_cast< DIGlobalVariable >(node);
    line = digv->getLine();
...
void GlobalReferences::translate_zero_initializer(ARCode& cfg,
  ...
  if (StructType* tstruct =
    ...
    for (unsigned i = 0; i < tstruct->getNumContainedTypes(); ++i) {
    ...
    for (unsigned idx = 0; idx < tarray->getNumElements(); ++idx) {
    ...
    store << store_lhs.expr() << store_rhs.expr() << get_debug_sexpr();
...
void GlobalReferences::translate_index(ARCode& cfg,
  ...
  if (StructType* tstruct =
    ...
    throw arbos_pass_error(
        "Unsupported type in GetElementPtr/ExtractValue/InsertValue");
  }
}
...
s_expression GlobalReferences::translate_getelementptr(ARCode& cfg,
  ...
  ++gti;
  for (unsigned op = 2; gti != gep_type_end(gep); ++gti, ++op) {
    Value* idx = gep->getOperand(op);
    translate_index(cfg, *gti, idx, base_offset, "ptrshift");
  }
...
s_expression GlobalReferences::translate_extractvalue(ARCode& cfg,
                                                      ExtractValueInst* ev) {
  s_expression_ostream o("loadelem");
  ...
  for (InsertValueInst::idx_iterator idx_it = ev->idx_begin();
       idx_it != ev->idx_end();
       ++idx_it) {
...
s_expression GlobalReferences::translate_insertvalue(ARCode& cfg,
                                                     InsertValueInst* iv) {
  s_expression_ostream o("storeelem");
  ...
  if (dyn_cast< BlockAddress >(cst)) {
    ...
    switch (e->getOpcode()) {
      ...
      case Instruction::ExtractValue: {
        ArrayRef< unsigned > indices = e->getIndices();
...
s_expression GlobalReferences::translate_type(const Type* t) {
  ...
  IntegerType* tint = const_cast< IntegerType* >(dyn_cast< IntegerType >(t));
  FunctionType* tfunc =
      const_cast< FunctionType* >(dyn_cast< FunctionType >(t));
  StructType* tstruct = const_cast< StructType* >(dyn_cast< StructType >(t));
  ArrayType* tarray = const_cast< ArrayType* >(dyn_cast< ArrayType >(t));
  PointerType* tptr = const_cast< PointerType* >(dyn_cast< PointerType >(t));
  VectorType* tvec = const_cast< VectorType* >(dyn_cast< VectorType >(t));
  ...
  if (tint != 0) {
    ...
    unsigned int i;
    for (i = 0; i < tfunc->getNumParams(); i++) {
    ...
    s << f.expr();
  } else if (tstruct != 0 && tstruct->isOpaque()) {
    ...
    throw arbos_pass_error("Unsupported type: vector");
...
s_expression gen_assertion(ARCode& cfg, CmpInst* inst, bool condition) {
  ...
  unsigned int nops = inst->getNumOperands();
  unsigned int i;
...
s_expression_ref ARFunction::gen_sexpr_intrinsic_call(ARCode& cfg,
  ...
  unsigned id = called_function->getIntrinsicID();
  ...
  if (id == Intrinsic::memset || id == Intrinsic::memcpy ||
    ...
    dest << refs->get_value(cfg, call->getArgOperand(0));
    meminst << dest.expr();
    ...
    s_expression_ostream len("len");
    len << refs->get_value(cfg, call->getArgOperand(2));
    meminst << len.expr();

    s_expression_ostream align("align");
    align << refs->get_value(cfg, call->getArgOperand(3));
    meminst << align.expr();

    s_expression_ostream isvolatile("isvolatile");
    isvolatile << refs->get_value(cfg, call->getArgOperand(4));
    ...
    return meminst.expr();
             ...
             id == Intrinsic::vacopy) {
    // XXX - TO BE COMPLETED: SUPPORT FOR llvm.va_start, llvm.va_end and
    // llvm.va_copy
    switch (id) {
    ...
    std::vector< s_expression_ref > arg_operands;
    unsigned int i;
    for (i = 0; i < call->getNumArgOperands(); i++) {
...
Instruction* find_next_dbg_branch(BasicBlock* curr,
                                  BasicBlock* dest,
                                  std::vector< BasicBlock* >& visited) {
  visited.push_back(curr);
  BranchInst* next_br = dyn_cast< BranchInst >(dest->getTerminator());
  if (next_br && next_br->getDebugLoc()) {
    return next_br;
  } else if (next_br &&
             std::find(visited.begin(), visited.end(), dest) == visited.end()) {
    return find_next_dbg_branch(dest, next_br->getSuccessor(0), visited);
  } else {
    return NULL;
  }
}

Instruction* find_next_dbg_branch(BasicBlock* curr, BasicBlock* dest) {
  std::vector< BasicBlock* > visited;
  return find_next_dbg_branch(curr, dest, visited);
}
...
s_expression_ref ARFunction::translate_instruction(Instruction* inst) {
  switch (inst->getOpcode()) {
    ...
    case Instruction::Invoke: { // Method call
      ...
      std::vector< s_expression_ref > arg_operands;
      unsigned int i;
      for (i = 0; i < invoke->getNumArgOperands(); i++) {
    ...
    case Instruction::Br: {
      ...
      if (br->isUnconditional()) {
        ...
        Instruction *bb_true_terminator = NULL, *bb_false_terminator = NULL;
        if (!br->getDebugLoc()) {
          bb_true_terminator =
              find_next_dbg_branch(br->getParent(), br->getSuccessor(0));
          bb_false_terminator =
              find_next_dbg_branch(br->getParent(), br->getSuccessor(1));
        }

        if (bb_true_terminator) {
          refs->set_debug_info(bb_true_terminator);
        }
        ...
        if (bb_false_terminator)
          refs->set_debug_info(bb_false_terminator);
        ...
        if (bb_true_terminator || bb_false_terminator)
          refs->reset_debug_info();
      }
    } break;
    ...
    case Instruction::Alloca: { // Stack management
      ...
      s_expression_ostream a("local_var");
      a << create_var_sexpr(alloca->getName().str(),
                            refs->get_type_refid(alloca->getType()));
      _local_vars.push_back(a.expr());
    } break;
    ...
    case Instruction::Store: {
      ...
      s_expression_ostream rhs("rhs");
      rhs << refs->get_value(_cfg, inst->getOperand(0));
      op << lhs.expr() << rhs.expr() << refs->get_debug_sexpr();
      return op.expr();
    } break;
