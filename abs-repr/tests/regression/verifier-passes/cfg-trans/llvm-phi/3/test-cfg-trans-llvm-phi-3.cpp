...
void Verifier::nodeStart(AR_Function& f) {
  ...
  std::string trans =
      "($trans\
     ($edge ($*_bb_3_split_1_icmp_true) ($*out_bb_3_merge_1_icmp))\
     ($edge ($bb_2) ($*in_bb_2_to_bb_3_phi))\
     ($edge ($*_bb_3_split_icmp_true) ($*out_bb_3_merge_icmp))\
     ($edge ($*out_bb_3_merge_icmp) ($*_bb_3_split_1_icmp_true))\
     ($edge ($*out_bb_3_merge_icmp) ($*_bb_3_split_1_icmp_false))\
     ($edge ($bb_3) ($*_bb_3_split_icmp_true))\
     ($edge ($bb_3) ($*_bb_3_split_icmp_false))\
     ($edge ($*in_bb_1_to_bb_3_phi) ($bb_3))\
     ($edge ($*in_bb_2_to_bb_3_phi) ($bb_3))\
     ($edge ($*_bb_3_split_icmp_false) ($*out_bb_3_merge_icmp))\
     ($edge ($*_bb_3_split_1_icmp_false) ($*out_bb_3_merge_1_icmp))\
     ($edge ($*out_bb_1_to_bb_3_icmp_false) ($*in_bb_1_to_bb_3_phi))\
     ($edge ($*out_bb_1_to_bb_2_icmp_true) ($bb_2))\
     ($edge ($bb_1) ($*out_bb_1_to_bb_2_icmp_true))\
     ($edge ($bb_1) ($*out_bb_1_to_bb_3_icmp_false))\
  )";
