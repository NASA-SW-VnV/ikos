/*******************************************************************************
 *
 * ARBOS pass for regression test parsing/call-args
 *
 * Authors: Nija Shi
 *
...
public:
  VisitorPass()
      : Pass("unittest-parsing-call-args",
             "Verifier pass for unittest-parsing-call-args"){};
...
void Verifier::nodeStart(AR_Call& call) {
  std::vector< AR_Node_Ref< AR_Operand > > args = call.getArguments();
  assert(args.size() == 1);
  AR_Node_Ref< AR_Var_Operand > var_operand =
      node_cast< AR_Var_Operand >(args[0]);
  AR_Node_Ref< AR_Internal_Variable > var =
      (*var_operand).getInternalVariable();
  assert((*var).getName() == "argc");
}
}
