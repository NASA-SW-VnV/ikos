...
void FunctionImporter::translate_instruction(
  ...
  } else if (auto va_arg_inst = llvm::dyn_cast< llvm::VAArgInst >(inst)) {
    this->translate_va_arg(bb_translation, va_arg_inst);
...
void FunctionImporter::translate_intrinsic_call(
  ...
  if (_ctx.bundle_imp->ignore_intrinsic(call->getIntrinsicID())) {
    ...
    // VarArgGet has no intrinsic; the llvm::VAArgInst instruction is
    // translated separately by translate_va_arg (Clang emits it on LLVM 18+
    // at -O0; older Clang lowered varargs to target-specific loads instead).
...
void FunctionImporter::translate_va_arg(BasicBlockTranslation* bb_translation,
                                        llvm::VAArgInst* inst) {
  ...
  // Translate the va_list pointer operand. Use the same void_ptr_ty idiom
  // (si8*) used by the va_start / va_end / va_copy translations above.
  ar::IntegerType* si8_ty = ar::IntegerType::si8(this->_context);
  ar::PointerType* void_ptr_ty = ar::PointerType::get(this->_context, si8_ty);
  ar::Value* operand =
      this->translate_value(bb_translation,
                            inst->getPointerOperand(),
                            void_ptr_ty);

  auto stmt = ar::VarArgGet::create(this->_bundle, var, operand);
