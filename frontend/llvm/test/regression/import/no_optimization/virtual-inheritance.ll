...
@_ZTT1D = linkonce_odr unnamed_addr constant [1 x i8*] [i8* bitcast (i8** getelementptr inbounds ({ [4 x i8*] }, { [4 x i8*] }* @_ZTV1D, i32 0, inrange i32 0, i32 4) to i8*)], align 8

@_ZTT1E = linkonce_odr unnamed_addr constant [1 x i8*] [i8* bitcast (i8** getelementptr inbounds ({ [4 x i8*] }, { [4 x i8*] }* @_ZTV1E, i32 0, inrange i32 0, i32 3) to i8*)], align 8

@_ZTT1F = linkonce_odr unnamed_addr constant [1 x i8*] [i8* bitcast (i8** getelementptr inbounds ({ [3 x i8*] }, { [3 x i8*] }* @_ZTV1F, i32 0, inrange i32 0, i32 3) to i8*)], align 8

@_ZTT1G = linkonce_odr unnamed_addr constant [1 x i8*] [i8* bitcast (i8** getelementptr inbounds ({ [3 x i8*] }, { [3 x i8*] }* @_ZTV1G, i32 0, inrange i32 0, i32 3) to i8*)], align 8
...
; CHECK: }
; CHECK: }
; CHECK: define [1 x si8*]* @_ZTT1D, align 8, init {
; CHECK: #1 !entry !exit {
; CHECK:   si8* %1 = ptrshift @_ZTV1D, 32 * 0, 1 * 0, 8 * 4
; CHECK:   store @_ZTT1D, [%1], align 1
; CHECK: }
; CHECK: }
; CHECK: define [1 x si8*]* @_ZTT1E, align 8, init {
; CHECK: #1 !entry !exit {
; CHECK:   si8* %1 = ptrshift @_ZTV1E, 32 * 0, 1 * 0, 8 * 3
; CHECK:   store @_ZTT1E, [%1], align 1
; CHECK: }
; CHECK: }
; CHECK: define [1 x si8*]* @_ZTT1F, align 8, init {
; CHECK: #1 !entry !exit {
; CHECK:   si8* %1 = ptrshift @_ZTV1F, 24 * 0, 1 * 0, 8 * 3
; CHECK:   store @_ZTT1F, [%1], align 1
; CHECK: }
; CHECK: }
; CHECK: define [1 x si8*]* @_ZTT1G, align 8, init {
; CHECK: #1 !entry !exit {
; CHECK:   si8* %1 = ptrshift @_ZTV1G, 24 * 0, 1 * 0, 8 * 3
; CHECK:   store @_ZTT1G, [%1], align 1
; CHECK: }
; CHECK: }
...
; CHECK:   si8* %7 = load %6, align 8
; CHECK:   si8** %8 = bitcast $4
; CHECK:   si8* %9 = load %8, align 8
; CHECK:   si8** %10 = bitcast %9
; CHECK:   si8* %11 = load %10, align 8
; CHECK:   si8* %12 = bitcast %7
; CHECK:   si8* %13 = bitcast %11
; CHECK:   si8** %14 = bitcast %12
...
; CHECK: #1 !entry !exit {
; CHECK:   {0: <{0: si8*, 8: si32}>, 16: {0: si8*, 8: si8*}, 32: {0: si32}}** $2 = allocate {0: <{0: si8*, 8: si32}>, 16: {0: si8*, 8: si8*}, 32: {0: si32}}*, 1, align 8
; CHECK:   store $2, %1, align 8
; CHECK:   si8** %3 = bitcast $2
; CHECK:   si8* %4 = load %3, align 8
; CHECK:   si8* %5 = bitcast %4
...
; CHECK:   si8** %17 = bitcast %14
; CHECK:   store %17, %16, align 8
; CHECK:   si8* %18 = bitcast %4
; CHECK:   si8* %19 = ptrshift %18, 1 * 16
; CHECK:   si8* %20 = bitcast %19
; CHECK:   si8* %21 = ptrshift @_ZTV1H, 48 * 0, 1 * 24, 8 * 3
; CHECK:   si8* %22 = bitcast %21
; CHECK:   si8** %23 = bitcast %20
; CHECK:   store %23, %22, align 8
; CHECK:   return
; CHECK: }
; CHECK: }
...
; CHECK:   {0: <{0: si8*, 8: si32}>, 16: {0: si8*, 8: si8*}, 32: {0: si32}}* $6 = allocate {0: <{0: si8*, 8: si32}>, 16: {0: si8*, 8: si8*}, 32: {0: si32}}, 1, align 8
; CHECK:   si32* %7 = bitcast $1
