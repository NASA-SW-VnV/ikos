 ...
 *
 * \file
 * \brief A dummy uninitialized abstract domain that is either top or bottom
 *
...
/// \brief Dummy uninitialized abstract domain that is either top or bottom
///
/// This can be used to disable an uninitialized variable analysis in a generic
/// implementation
...
public:
  ...
  void assign_uninitialized(VariableRef) override {}
  ...
  void refine(VariableRef, const Uninitialized& value) override {
    if (value.is_bottom()) {
      this->_is_bottom = true;
    }
  }
  ...
  static std::string name() { return "dummy uninitialized domain"; }
