...
#include <ikos/core/domain/bitwise_operator.hpp>
#include <ikos/core/domain/counter_domain.hpp>
#include <ikos/core/domain/division_operator.hpp>
#include <ikos/core/domain/memory_domain.hpp>
#include <ikos/core/domain/nullity.hpp>
#include <ikos/core/domain/numerical_domain.hpp>
#include <ikos/core/domain/pointer.hpp>
...
public:
  ...
  Cell(VariableFactory& vfac,
      ...
      : _base(base),
        ...
        _type(type) {
    ikos_assert(!range.is_bottom() && range.lb().number());
    _scalar_var =
        var_name_traits::summary_domain_cell_var< VariableFactory,
                                                  MemoryLocation,
                                                  Number >()(vfac,
  ...
  void write(std::ostream& o) {
    o << "C{";
    DumpableTraits< MemoryLocation >::dump(o, _base);
...
public:
  ...
  typedef DummyNumber floating_point_t;
  ...
  typedef LinearConstraintSystem< Number, VariableName >
      linear_constraint_system_t;
  typedef discrete_domain< MemoryLocation > points_to_set_t;
  typedef literal< Number, DummyNumber, VariableName > literal_t;
...
private:
  ...
  Cell realize_single_out_cell(VariableFactory& vfac,
    ...
    if (!found) { // no cell found for base
      ...
      if (cells.find(c) != cells.end()) { // same cell found
        ...
        // TODO(marthaud): perform a reduction to improve precision
        add_cell(c);
        return c;
      }
    }
  ...
  std::vector< Cell > realize_range_out_cells(VariableFactory& vfac,
    ...
    ikos_assert(!offset_intv.is_bottom());
  ...
  public:
    ...
    void floating_point(DummyNumber) {
      _ptr -= _lhs; // ignored
      _uninitialized.make_initialized(_lhs);
      _nullity -= _lhs;
    }
    ...
    void integer_var(VariableName rhs) {
      _ptr.assign(_lhs, linear_expression_t(rhs));
      _ptr.forget_pointer(_lhs);
  ...
  public:
    ...
    void integer(Number) {
      throw LogicError("value_domain: trying to assign an integer");
    }

    void floating_point(DummyNumber) {
      throw LogicError("value_domain: trying to assign a floating point");
    }

    void undefined() {
      throw LogicError("value_domain: trying to assign undefined");
    }

    void null() { throw LogicError("value_domain: trying to assign null"); }

    void integer_var(VariableName lhs) {
      _ptr.assign(lhs, linear_expression_t(_rhs));
      _ptr.forget_num(lhs);
  ...
  struct merge_cells_op : public var_to_cell_set_t::generic_binary_op_t {
    ...
    boost::optional< cell_set_t > apply(cell_set_t left, cell_set_t right) {
      ikos_assert(!left.empty() && !right.empty());
  ...
  static bool leq_cells(var_to_cell_set_t left_cells,
    ...
    for (auto it = left_cells.begin(); it != left_cells.end(); ++it) {
      variables += it->first;
    }
    for (auto it = right_cells.begin(); it != right_cells.end(); ++it) {
      variables += it->first;
    }
...
public:
  ...
  void assign(VariableName x, int n) {
    if (is_top())
      return;
  ...
  void assign(VariableName x, Number n) {
    if (is_top())
      return;
  ...
  void assign(VariableName x, const linear_expression_t& e) {
    if (is_top())
      return;
  ...
  interval_t to_interval(const linear_expression_t& e, bool normalize) {
    if (is_top())
      return interval_t::top();
  ...
  void assign_pointer(VariableName p,
                      VariableName q,
                      const linear_expression_t& o) {
    if (is_top())
      return;
  ...
  void mem_copy(VariableFactory& vfac,
                VariableName dest,
                VariableName src,
                const linear_expression_t& size) {
    if (is_top() || is_bottom())
      return;
    ...
    if (dest_addrs.size() == 1 && dest_intv.singleton() &&
      ...
      for (auto it = src_addrs.begin(); it != src_addrs.end(); ++it) {
  ...
  void mem_set(VariableFactory& vfac,
               VariableName dest,
               const linear_expression_t& value,
               const linear_expression_t& size) {
    if (is_top() || is_bottom())
      return;
  ...
  void write(std::ostream& o) {
    if (is_top()) {
      ...
      for (auto it = _cells.begin(); it != _cells.end();) {
        ...
        for (auto it2 = cells.begin(); it2 != cells.end();) {
          o << *it2;
          ++it2;
          if (it2 != cells.end())
            o << ", ";
...
struct lin_expr_to_interval_impl<
  ...
  inline interval< typename ScalarDomain::number_t > operator()(
      summary_domain< ScalarDomain, VariableFactory, MemoryLocation >& inv,
      const typename ScalarDomain::linear_expression_t& e,
      bool normalize) {
