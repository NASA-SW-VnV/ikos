...
class binary_operation final : public writeable {
public:
  typedef VariableExpression< Number, VariableName > variable_t;
...
template < typename VariableName, typename CheckPointName >
class z_binary_operation final
    : public statement< VariableName, CheckPointName > {
public:
  typedef VariableExpression< ZNumber, VariableName > variable_t;
  typedef binary_operation< ZNumber, VariableName, CheckPointName >
...
public:
  typedef VariableExpression< Number, VariableName > variable_t;
  typedef LinearExpression< Number, VariableName > linear_expression_t;
...
public:
  linear_assignment(variable_t lhs, linear_expression_t rhs)
      : _lhs(lhs), _rhs(std::move(rhs)) {}

  variable_t lhs() { return this->_lhs; }

  const linear_expression_t& rhs() const { return this->_rhs; }
...
template < typename VariableName, typename CheckPointName >
class z_linear_assignment final
    : public statement< VariableName, CheckPointName > {
public:
  typedef VariableExpression< ZNumber, VariableName > variable_t;
  typedef LinearExpression< ZNumber, VariableName > z_linear_expression_t;
  typedef linear_assignment< ZNumber, VariableName, CheckPointName >
...
public:
  typedef VariableExpression< Number, VariableName > variable_t;
  typedef LinearConstraint< Number, VariableName > linear_constraint_t;
...
public:
  linear_assertion(linear_constraint_t cst) : _cst(std::move(cst)) {}

  const linear_constraint_t& constraint() const { return this->_cst; }
...
template < typename VariableName, typename CheckPointName >
class z_linear_assertion final
    : public statement< VariableName, CheckPointName > {
public:
  typedef VariableExpression< ZNumber, VariableName > variable_t;
  typedef LinearConstraint< ZNumber, VariableName > z_linear_constraint_t;
  typedef linear_assertion< ZNumber, VariableName, CheckPointName >
...
public:
  z_linear_assertion(z_linear_constraint_t cst) : _stmt(std::move(cst)) {}
...
template < typename VariableName, typename CheckPointName >
class checkpoint final : public statement< VariableName, CheckPointName > {
private:
...
public:
  array_write(variable_t array,
      ...
      : _array(array), _index(std::move(index)), _val(std::move(val)) {}

  variable_t array() { return this->_array; }

  const linear_expression_t& index() const { return this->_index; }

  const linear_expression_t& val() const { return this->_val; }
...
template < typename VariableName, typename CheckPointName >
class z_array_write final : public statement< VariableName, CheckPointName > {
public:
  typedef VariableExpression< ZNumber, VariableName > variable_t;
  typedef LinearExpression< ZNumber, VariableName > z_linear_expression_t;
  typedef array_write< ZNumber, VariableName, CheckPointName > z_array_write_t;
...
public:
  z_array_write(variable_t array,
      ...
      : _stmt(array, std::move(index), std::move(value)) {}
...
public:
  array_read(variable_t lhs, variable_t array, linear_expression_t index)
      : _lhs(lhs), _array(array), _index(std::move(index)) {}
...
template < typename VariableName, typename CheckPointName >
class z_array_read final : public statement< VariableName, CheckPointName > {
public:
  typedef VariableExpression< ZNumber, VariableName > variable_t;
  typedef LinearExpression< ZNumber, VariableName > z_linear_expression_t;
  typedef array_read< ZNumber, VariableName, CheckPointName > z_array_read_t;
...
public:
  z_array_read(variable_t lhs, variable_t array, z_linear_expression_t index)
      : _stmt(lhs, array, std::move(index)) {}
...
public:
  ...
  typedef VariableExpression< ZNumber, VariableName > z_variable_t;
  typedef LinearExpression< ZNumber, VariableName > z_linear_expression_t;
  typedef LinearConstraint< ZNumber, VariableName > z_linear_constraint_t;
...
public:
  ...
  void assign(z_variable_t lhs, z_linear_expression_t rhs) {
    this->add(std::static_pointer_cast< statement_t, z_linear_assignment_t >(
        z_linear_assignment_ptr(
            new z_linear_assignment_t(lhs, std::move(rhs)))));
  }
  ...
  void write(z_variable_t array,
    ...
    this->add(std::static_pointer_cast< statement_t, z_array_write_t >(
        z_array_write_ptr(
            new z_array_write_t(array, std::move(index), std::move(value)))));
  }
  ...
  void read(z_variable_t lhs, z_variable_t array, z_linear_expression_t index) {
    this->add(std::static_pointer_cast< statement_t, z_array_read_t >(
        z_array_read_ptr(new z_array_read_t(lhs, array, std::move(index)))));
  }

  void assertion(z_linear_constraint_t cst) {
    this->add(std::static_pointer_cast< statement_t, z_linear_assertion_t >(
        z_linear_assertion_ptr(new z_linear_assertion_t(std::move(cst)))));
  }
  ...
  private:
    ...
    basic_block_t& dereference() const {
      if (this->_it != this->_ids->end()) {
        ...
        throw LogicError("muAZ cfg: trying to dereference an empty iterator");
      }
    }
  }; // class iterator
...
public:
  ...
  std::size_t size() { return this->_blocks->size(); }
