...
#ifndef IKOS_EXCEPTION_HPP
#define IKOS_EXCEPTION_HPP
...
public:
  ...
  explicit logic_error(const std::string& msg) : exception(), _msg(msg) {}

  /// \brief Constructor
  ///
  /// \param msg Explanatory message
  explicit logic_error(const char* msg) : exception(), _msg(msg) {}

  /// \brief Remove the default constructor
  logic_error() = delete;

  /// \brief Copy constructor
  logic_error(const logic_error& o) = default;

  /// \brief Get the explanatory string
  virtual const char* what() const noexcept { return this->_msg.c_str(); }

}; // end class logic_error

} // end namespace ikos

#endif // IKOS_EXCEPTION_HPP
