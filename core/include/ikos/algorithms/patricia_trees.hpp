...
public:
  ...
  index64_t prefix() const { return this->_key.index(); }
...
typename tree< Key, Value >::ptr tree< Key, Value >::generic_merge(
  ...
  if (s) {
    if (t) {
      if (s->is_leaf()) {
        if (t->is_leaf()) {
          if (s->binding().first.index() == t->binding().first.index()) {
            boost::optional< Value > new_value =
                combine_left_to_right
          ...
          index64_t branching_bit = t->branching_bit();
          index64_t prefix = t->prefix();
          if (match_prefix(s->binding().first.index(), prefix, branching_bit)) {
            if (zero_bit(s->binding().first.index(), branching_bit)) {
              tree_ptr new_lb =
