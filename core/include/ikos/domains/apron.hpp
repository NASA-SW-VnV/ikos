...
// wrapper for ap_abstract0_t*
typedef std::shared_ptr< ap_abstract0_t > inv_ptr_t;

// deleter for inv_ptr_t
struct inv_deleter {
...
// remove some dimensions of a ap_abstract0_t
...
// conversion from ikos::z_number/q_number to ap_texpr0_t*
template < typename Number >
ap_texpr0_t* to_ap_expr(Number);

template <>
ap_texpr0_t* to_ap_expr(z_number n) {
  mpq_class e(static_cast< mpz_class >(n));
  return ap_texpr0_cst_scalar_mpq(e.get_mpq_t());
}

template <>
ap_texpr0_t* to_ap_expr(q_number q) {
  mpq_class e(static_cast< mpq_class >(q));
  return ap_texpr0_cst_scalar_mpq(e.get_mpq_t());
}

// conversion from ap_scalar_t* to ikos::z_number/q_number
template < typename Number >
...
// conversion from ap_coeff_t* to ikos::z_number/q_number
template < typename Number >
...
// conversion from ap_scalar_t* to ikos::bound< Number >
template < typename Number >
...
// conversion from ap_interval_t* to ikos::interval< Number >
template < typename Number >
...
private:
  // get the dimension associated to a variable
  inline boost::optional< ap_dim_t > var_dim(VariableName v) const {
    return _var_map.lookup(v);
  }

  // get the dimension associated to a variable, or create one
  ...
  // unary operator for variable_map_t that removes a dimension
  ...
  // unary operator for variable_map_t that removes a list of dimensions
  ...
  // merge two variable maps, updating the associated abstract values
  ...
  // conversion from VariableName to ap_texpr0_t*
  ap_texpr0_t* to_ap_expr(VariableName v) {
    return ap_texpr0_dim(var_dim_insert(v));
  }

  // conversion from linear_expression_t to ap_texpr0_t*
  ...
  // conversion from linear_constraint_t to ap_tcons0_t
  ...
  // conversion from ap_linexpr0_t* to linear_expression_t
  ...
  // conversion from ap_lincons0_t to linear_constraint_t
...
public:
  ...
  void set(VariableName v, interval_t intv) {
    ...
    // add constraints inf <= v <= sup
    linear_constraint_system_t csts;

    if (intv.lb().is_finite()) {
      Number lb = *intv.lb().number();
      csts += (variable_t(v) >= lb);
    }
    if (intv.ub().is_finite()) {
      Number ub = *intv.ub().number();
      csts += (variable_t(v) <= ub);
    }
  ...
  void operator-=(variable_set_t variables) {
    ...
    vector_dims.reserve(variables.size());

    // gather all dimensions
    for (typename variable_set_t::iterator it = variables.begin();
         it != variables.end();
         ++it) {
      boost::optional< ap_dim_t > dim = var_dim(it->name());
