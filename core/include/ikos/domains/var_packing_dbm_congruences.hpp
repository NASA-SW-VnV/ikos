/*******************************************************************************
 *
 * Reduced product of dbm (using variable packing) and congruences.
 *
...
private:
  ...
  // Reduce the equivalence class containing the variable `v`.
  ...
  void reduce_equivalence_class(const VariableName& v, bool need_copy = true) {
    ...
    while ((change_dbm || change_congruence) &&
      ...
      for (typename variable_set_t::iterator it = variables.begin();
           it != variables.end();
           ++it) {
        interval_t i = subdomain->to_interval(*it);
        ...
        if (val.is_bottom()) {
          *this = bottom();
          ...
          if (val.first() != i) {
            ...
            if (val.first().ub().number()) {
              *subdomain += (*it <= *val.first().ub().number());
            }
            if (val.first().lb().number()) {
              *subdomain += (*it >= *val.first().lb().number());
            }
...
public:
  ...
  // convert an interval_congruence_domain to a var_packing_dbm_congruence
  explicit var_packing_dbm_congruence(interval_congruence_domain_t o)
      : _product(var_packing_dbm_t(o.first()), o.second()) {}

  // convert a var_packing_dbm_congruence to an interval_congruence_domain
  ...
  interval_congruence_t operator[](VariableName v) {
    return interval_congruence_t(_product.first()[v], _product.second()[v]);
  }
  ...
  void operator-=(variable_set_t vars) { _product -= vars; }
  ...
  void apply(conv_operation_t op,
             ...
             VariableName y,
             unsigned width) {
    _product.apply(op, x, y, width);
    reduce_equivalence_class(x, false);
  }

  void apply(conv_operation_t op, VariableName x, Number k, unsigned width) {
    _product.apply(op, x, k, width);
    reduce_equivalence_class(x, false);
  }
