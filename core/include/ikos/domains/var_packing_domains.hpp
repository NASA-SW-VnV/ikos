...
public:
  ...
  friend class var_packing_dbm< Number, VariableName >;
...
public:
  ...
  // convert an interval_domain to a var_packing_domain
  ...
  // convert a var_packing_domain to an interval_domain
  interval_domain_t get_interval_domain() {
    ...
    for (typename variable_set_t::iterator it = vars.begin(); it != vars.end();
         ++it) {
      VariableName v = (*it).name();
      result.set(v, _equiv_relation.find_domain(v)->operator[](v));
    }
