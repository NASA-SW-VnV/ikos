/*******************************************************************************
 *
 * Generic API for abstract domains that keep track of (non-)null variables.
 *
...
inline bool is_null(Domain& inv,
                    ...
                    typename enable_if_not_nullity< Domain >::type* = 0) {
  if (inv.is_bottom()) {
    throw ikos_error("null_domain_traits: trying to call is_null() on bottom");
  } else {
    return false;
  }
}
