/*******************************************************************************
 *
 * Generic API for abstract domains that keep track of (un)initialized
 * variables.
 *
...
public:
  ...
  // assign x = f(y[0], y[1], ..., y[n])
  virtual void assign_uninitialized(VariableName x,
                                    const std::vector< VariableName >& y) = 0;

  // return true iff v is initialized
  virtual bool is_initialized(VariableName v) = 0;

  // return true iff v is uninitialized
...
inline void assign_uninitialized(
    Domain& inv,
    typename Domain::variable_name_t x,
    const std::vector< typename Domain::variable_name_t >& y,
    typename enable_if_uninitialized< Domain >::type* = 0) {
  inv.assign_uninitialized(x, y);
}
