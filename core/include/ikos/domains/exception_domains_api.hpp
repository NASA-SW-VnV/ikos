/*******************************************************************************
 *
 * Generic API for abstract domains that keep track of exceptions.
 *
...
public:
   ...
   *
   * Arguments:
   *   exc: The result variable that contains the exception structure (exception
   *        pointer and exception type id)
   */
  virtual void enter_catch(VariableName exc) = 0;

  // disregard the pending exception
   ...
   *
   * Arguments:
   *   exc: The exception pointer
   *   tinfo: The pointer to the exception type_info
   *   dest: The pointer to the destructor of the exception (or null)
   */
  virtual void throw_exception(VariableName exc,
                               VariableName tinfo,
                               boost::optional< VariableName > dest) = 0;

  /* Resume/rethrow an exception
   *
   * Arguments:
   *   exc: The exception structure (exception pointer and exception type id)
   */
  virtual void resume_exception(VariableName exc) = 0;
