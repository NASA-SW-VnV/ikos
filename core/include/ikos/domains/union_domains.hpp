...
private:
  ...
  public:
    ...
    Domain value() const {
      throw ikos_error("union_domain: trying to call value() on a node");
    }
  ...
  public:
    ...
    const tree_ptr& left_branch() const {
      throw ikos_error("union_domain: trying to call left_branch() on a leaf");
    }
    const tree_ptr& right_branch() const {
      throw ikos_error("union_domain: trying to call right_branch() on a leaf");
    }
  ...
  // reduce the height of a tree by merging the nodes at a heigh greater than
  // max_height
