...
typedef interval_congruence< z_number > interval_congruence_t;
typedef var_packing_dbm_congruence< z_number, varname_t > domain_t;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout << "# Test interval projection ##################################"
              << std::endl;
    interval_congruence_t interval = interval_congruence_t::top();
    ...
    inv += (z_var(x) == 4 * z_var(a));
    std::cout << "After x = 4 * a:\n\t" << inv << std::endl;
