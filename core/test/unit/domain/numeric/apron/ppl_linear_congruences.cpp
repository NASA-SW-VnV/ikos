/*******************************************************************************
 *
 * Tests for ApronDomain with Domain = PplLinearCongruences
 *
...
#define BOOST_TEST_MODULE test_apron_ppl_linear_congruences
...
// TODO(marthaud): some tests are commented out, this is because we need to
// improve to_interval() and to_congruence() methods in the ApronDomain to make
// them pass.
...
using ApronDomain = ikos::core::numeric::ApronDomain<
    ikos::core::numeric::apron::PplLinearCongruences,
    ZNumber,
    Variable >;
...
BOOST_AUTO_TEST_CASE(widening) {
  ...
  // TOP widen inv1 gives {x = 0 mod 1}, and is not reduced to top.
  BOOST_CHECK((ApronDomain::top().widening(inv1).to_congruence(x) ==
               Congruence::top()));
