...
#define BOOST_TEST_MODULE test_apron_pkgrid_polyhedra_lin_congruences
...
using ApronDomain = ikos::core::numeric::ApronDomain<
    ikos::core::numeric::apron::PkgridPolyhedraLinCongruences,
    ZNumber,
    Variable >;
...
BOOST_AUTO_TEST_CASE(refine) {
  ...
  inv.refine(x, Congruence(ZNumber(3), ZNumber(1)));
  // Apron Pkgrid Polyhedra Lin Congruences might not infer 4 <= x <= 7
  BOOST_CHECK(inv.to_interval(x) == Interval(Bound(2), Bound(9)) ||
              inv.to_interval(x) == Interval(Bound(4), Bound(7)));
