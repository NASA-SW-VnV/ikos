...
#define BOOST_TEST_MODULE test_var_packing_domain
...
using VarPackingDomain =
    ikos::core::numeric::VarPackingDomain< ZNumber, Variable, DBM >;
...
BOOST_AUTO_TEST_CASE(is_top_and_bottom) {
  ...
  BOOST_CHECK(VarPackingDomain::top().is_top());
  BOOST_CHECK(!VarPackingDomain::top().is_bottom());

  BOOST_CHECK(!VarPackingDomain::bottom().is_top());
  BOOST_CHECK(VarPackingDomain::bottom().is_bottom());
...
BOOST_AUTO_TEST_CASE(leq) {
  ...
  BOOST_CHECK(!VarPackingDomain::top().leq(VarPackingDomain::bottom()));
  BOOST_CHECK(VarPackingDomain::top().leq(VarPackingDomain::top()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(0));
  BOOST_CHECK(inv1.leq(VarPackingDomain::top()));
  BOOST_CHECK(!inv1.leq(VarPackingDomain::bottom()));

  auto inv2 = VarPackingDomain::top();
  inv2.set(x, Interval(Bound(-1), Bound(1)));
  BOOST_CHECK(inv2.leq(VarPackingDomain::top()));
  BOOST_CHECK(!inv2.leq(VarPackingDomain::bottom()));
  BOOST_CHECK(inv1.leq(inv2));
  BOOST_CHECK(!inv2.leq(inv1));

  auto inv3 = VarPackingDomain::top();
  inv3.set(x, Interval(0));
  inv3.set(y, Interval(Bound(-1), Bound(1)));
  BOOST_CHECK(inv3.leq(VarPackingDomain::top()));
  BOOST_CHECK(!inv3.leq(VarPackingDomain::bottom()));
  BOOST_CHECK(inv3.leq(inv1));
  BOOST_CHECK(!inv1.leq(inv3));

  auto inv4 = VarPackingDomain::top();
  inv4.set(x, Interval(0));
  inv4.set(y, Interval(Bound(0), Bound(2)));
  BOOST_CHECK(inv4.leq(VarPackingDomain::top()));
  BOOST_CHECK(!inv4.leq(VarPackingDomain::bottom()));
  BOOST_CHECK(!inv3.leq(inv4));
  BOOST_CHECK(!inv4.leq(inv3));

  auto inv5 = VarPackingDomain::top();
  inv5.set(x, Interval(0));
  inv5.set(y, Interval(Bound(0), Bound(2)));
  inv5.set(z, Interval(Bound::minus_infinity(), Bound(0)));
  BOOST_CHECK(inv5.leq(VarPackingDomain::top()));
  BOOST_CHECK(!inv5.leq(VarPackingDomain::bottom()));
...
BOOST_AUTO_TEST_CASE(equals) {
  ...
  BOOST_CHECK(VarPackingDomain::top().equals(VarPackingDomain::top()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(0));
  BOOST_CHECK(!inv1.equals(VarPackingDomain::top()));
  BOOST_CHECK(!inv1.equals(VarPackingDomain::bottom()));
  BOOST_CHECK(inv1.equals(inv1));

  auto inv2 = VarPackingDomain::top();
  inv2.set(x, Interval(Bound(-1), Bound(1)));
  BOOST_CHECK(!inv2.equals(VarPackingDomain::top()));
  BOOST_CHECK(!inv2.equals(VarPackingDomain::bottom()));
  ...
  inv3.set(x, Interval(0));
  inv3.set(y, Interval(Bound(-1), Bound(1)));
  BOOST_CHECK(!inv3.equals(VarPackingDomain::top()));
  BOOST_CHECK(!inv3.equals(VarPackingDomain::bottom()));
...
BOOST_AUTO_TEST_CASE(join) {
  ...
  BOOST_CHECK((VarPackingDomain::bottom().join(VarPackingDomain::bottom()) ==
               VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::top().join(VarPackingDomain::top()) ==
               VarPackingDomain::top()));
  BOOST_CHECK((VarPackingDomain::top().join(VarPackingDomain::bottom()) ==
               VarPackingDomain::top()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(Bound(0), Bound(1)));
  BOOST_CHECK((inv1.join(VarPackingDomain::top()) == VarPackingDomain::top()));
  BOOST_CHECK((inv1.join(VarPackingDomain::bottom()) == inv1));
  BOOST_CHECK((VarPackingDomain::top().join(inv1) == VarPackingDomain::top()));
...
BOOST_AUTO_TEST_CASE(widening) {
  ...
  BOOST_CHECK((VarPackingDomain::bottom().widening(VarPackingDomain::top()) ==
               VarPackingDomain::top()));
  BOOST_CHECK((VarPackingDomain::bottom().widening(
                   VarPackingDomain::bottom()) == VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::top().widening(VarPackingDomain::top()) ==
               VarPackingDomain::top()));
  BOOST_CHECK((VarPackingDomain::top().widening(VarPackingDomain::bottom()) ==
               VarPackingDomain::top()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(Bound(0), Bound(1)));
  BOOST_CHECK(
      (inv1.widening(VarPackingDomain::top()) == VarPackingDomain::top()));
  BOOST_CHECK((inv1.widening(VarPackingDomain::bottom()) == inv1));
  BOOST_CHECK(
      (VarPackingDomain::top().widening(inv1) == VarPackingDomain::top()));
...
BOOST_AUTO_TEST_CASE(widening_threshold) {
  ...
  BOOST_CHECK(
      (VarPackingDomain::bottom().widening_threshold(VarPackingDomain::top(),
       ...
       VarPackingDomain::top()));
  BOOST_CHECK(
      (VarPackingDomain::bottom().widening_threshold(VarPackingDomain::bottom(),
                                                     ZNumber(10)) ==
       VarPackingDomain::bottom()));
  BOOST_CHECK(
      (VarPackingDomain::top().widening_threshold(VarPackingDomain::top(),
                                                  ZNumber(10)) ==
       VarPackingDomain::top()));
  BOOST_CHECK(
      (VarPackingDomain::top().widening_threshold(VarPackingDomain::bottom(),
                                                  ZNumber(10)) ==
       VarPackingDomain::top()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(Bound(0), Bound(1)));
  BOOST_CHECK((inv1.widening_threshold(VarPackingDomain::top(), ZNumber(10)) ==
               VarPackingDomain::top()));
  BOOST_CHECK((inv1.widening_threshold(VarPackingDomain::bottom(),
                                       ZNumber(10)) == inv1));
  BOOST_CHECK((VarPackingDomain::top().widening_threshold(inv1, ZNumber(10)) ==
               VarPackingDomain::top()));
  BOOST_CHECK(
      (VarPackingDomain::bottom().widening_threshold(inv1, ZNumber(10)) ==
       inv1));
  ...
  auto inv4 = VarPackingDomain::top();
  auto inv5 = VarPackingDomain::top();
  auto inv6 = VarPackingDomain::top();
  inv4.set(x, Interval(Bound(-1), Bound(0)));
...
BOOST_AUTO_TEST_CASE(narrowing_threshold) {
  ...
  BOOST_CHECK(
      (VarPackingDomain::bottom().narrowing_threshold(VarPackingDomain::top(),
       ...
       VarPackingDomain::bottom()));
  BOOST_CHECK((
      VarPackingDomain::bottom().narrowing_threshold(VarPackingDomain::bottom(),
                                                     ZNumber(10)) ==
      VarPackingDomain::bottom()));
  BOOST_CHECK(
      (VarPackingDomain::top().narrowing_threshold(VarPackingDomain::top(),
                                                   ZNumber(10)) ==
       VarPackingDomain::top()));
  BOOST_CHECK(
      (VarPackingDomain::top().narrowing_threshold(VarPackingDomain::bottom(),
                                                   ZNumber(10)) ==
       VarPackingDomain::bottom()));
  ...
  BOOST_CHECK(
      (inv1.narrowing_threshold(VarPackingDomain::top(), ZNumber(10)) == inv1));
  BOOST_CHECK(
      (inv1.narrowing_threshold(VarPackingDomain::bottom(), ZNumber(10)) ==
       VarPackingDomain::bottom()));
  BOOST_CHECK(
      (VarPackingDomain::top().narrowing_threshold(inv1, ZNumber(10)) == inv1));
  BOOST_CHECK(
      (VarPackingDomain::bottom().narrowing_threshold(inv1, ZNumber(10)) ==
       VarPackingDomain::bottom()));
...
BOOST_AUTO_TEST_CASE(meet) {
  ...
  BOOST_CHECK((VarPackingDomain::bottom().meet(VarPackingDomain::top()) ==
               VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::bottom().meet(VarPackingDomain::bottom()) ==
               VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::top().meet(VarPackingDomain::top()) ==
               VarPackingDomain::top()));
  BOOST_CHECK((VarPackingDomain::top().meet(VarPackingDomain::bottom()) ==
               VarPackingDomain::bottom()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(Bound(0), Bound(1)));
  BOOST_CHECK((inv1.meet(VarPackingDomain::top()) == inv1));
  BOOST_CHECK(
      (inv1.meet(VarPackingDomain::bottom()) == VarPackingDomain::bottom()));
  ...
  inv2.add(VariableExpr(x) <= 0);
  BOOST_CHECK(
      (inv1.meet(inv2) == VarPackingDomain::bottom())); // {x = 1} & {x <= 0}
...
BOOST_AUTO_TEST_CASE(narrowing) {
  ...
  BOOST_CHECK((VarPackingDomain::bottom().narrowing(VarPackingDomain::top()) ==
               VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::bottom().narrowing(
                   VarPackingDomain::bottom()) == VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::top().narrowing(VarPackingDomain::top()) ==
               VarPackingDomain::top()));
  BOOST_CHECK((VarPackingDomain::top().narrowing(VarPackingDomain::bottom()) ==
               VarPackingDomain::bottom()));

  auto inv1 = VarPackingDomain::top();
  inv1.set(x, Interval(Bound(0), Bound::plus_infinity()));
  BOOST_CHECK((inv1.narrowing(VarPackingDomain::top()) == inv1));
  BOOST_CHECK((inv1.narrowing(VarPackingDomain::bottom()) ==
               VarPackingDomain::bottom()));
  BOOST_CHECK((VarPackingDomain::top().narrowing(inv1) == inv1));
  BOOST_CHECK((VarPackingDomain::bottom().narrowing(inv1) ==
               VarPackingDomain::bottom()));
