 ...
 * possible node in the control flow graph. If we find a unreachable path,
...
class TransferFunVisitor : public arbos_visitor_api {
  typedef analyzer::num_sym_exec< AbsNumDomain, VariableName, Number >
      sym_exec_t;
...
public:
  TransferFunVisitor(AbsNumDomain pre,
      ...
      : _sym_exec(pre, vfac, lfac, analyzer::REG), _vfac(vfac) {}
  ...
  void visit(Call_ref s) {
    if (ar::getReturnValue(s)) {
      ...
      AbsNumDomain inv = _sym_exec.inv();
      inv -= lhs;
      _sym_exec.set_inv(inv);
    }
  }
...
private:
  ...
  typedef TransferFunVisitor< AbsNumDomain, VariableName, Number >
...
public:
  ...
  bool prune_edges_from(Basic_Block_ref node, Basic_Block_ref child) {
    ...
    if (inv.is_bottom()) {
      ...
      child->removePreviousBasicBlock(node);
      ...
      if (ar::getPreds(child).size() == 1) {
        ...
        for (unsigned int i = 0; i < succs.size(); i++) {
          if (!path_reachable[i]) {
            ...
            succs[i]->removePreviousBasicBlock(child);
        ...
        Basic_Block_ref clone_child(
            AR_Basic_Block::create(child->getContainingCode()->getUID(),
                                   clone_name)
                ->getUID());
  ...
  void merge_with_child(Basic_Block_ref node) {
    ...
    StmtRange stmts = ar::getStatements(child);
    for (StmtRange::iterator it = stmts.begin(); it != stmts.end(); ++it) {
      transformations::addBackStatement(node, *it);
    }
  ...
  static void remove_edges(Basic_Block_ref node) {
    ...
    for (BBRange::iterator it = preds.begin(); it != preds.end(); ++it) {
      ...
      (*it)->removeNextBasicBlock(node);
    }
    for (BBRange::iterator it = succs.begin(); it != succs.end(); ++it) {
      node->removeNextBasicBlock(*it);
      (*it)->removePreviousBasicBlock(node);
    }
  }
