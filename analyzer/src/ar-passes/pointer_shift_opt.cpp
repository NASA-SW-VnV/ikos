  ...
  void removeArithOps(Basic_Block_ref block,
    ...
    for (StmtRange::iterator I = stmts.begin(), E = stmts.end(); I != E; ++I) {
      ...
      if (ar::is_pointer_shift_stmt(S)) {
        ...
        Literal loffset = lfac[offset];
        Operand_ref base = ar::getBase(ps);
        Literal lbase = lfac[base];
        varname_set_t target = varname_set_t::bottom();
        if (loffset.is_var())
          target += loffset.get_var();
        if (lbase.is_var())
          target += lbase.get_var();
        if (!target.is_bottom()) {
          ...
          replaceStatement(block, S, node_cast< Statement >(ps));
        }
        prev_stmts.clear();
...
public:
  ...
  void execute(Bundle_ref bundle) {
    ...
    for (FuncRange::iterator fI = entries.begin(), fE = entries.end(); fI != fE;
         ++fI) {
      BBRange blks = ar::getBlocks(*fI);
      for (BBRange::iterator bI = blks.begin(), bE = blks.end(); bI != bE;
           ++bI) {
        Basic_Block_ref b = *bI;
        live_info::live_info_ptr live(new live_info(vfac, lfac, PTR));
        ar::accept(b, live);
