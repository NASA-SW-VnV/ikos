...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Regression tests for analyses with memory level of precision')
    ...
    t.add(Test('test-4-safe.c', 'test-4-safe.c', 'prover', 'safe', options=['--gv-init-all']))
    ...
    t.add(Test('test-7-unsafe.c', 'test-7-unsafe.c', 'prover', 'error',
               line_checks=[(18, 'ok'), (19, 'ok'), (42, 'ok'), (43, 'error')]))
    ...
    t.add(Test('test-9-unsafe.c', 'test-9-unsafe.c', 'prover', 'error',
               line_checks=[(24, 'error'), (26, 'error'), (32, 'ok')]))
    ...
    t.add(Test('test-16.c', 'test-16.c', 'prover', 'safe', expected='unsafe',
               line_checks=[(29, 'ok', 'warning'),
                            (30, 'ok'),
                            (31, 'ok', 'warning'),
                            (32, 'ok')]))
    t.add(Test('test-17.c', 'test-17.c', 'prover', 'safe', expected='unsafe',
               line_checks=[(27, 'ok', 'warning'),
                            (28, 'ok'),
                            (29, 'ok', 'warning'),
                            (30, 'ok')]))
    ...
    t.add(Test('test-24b.c', 'test-24b.c', 'prover', 'safe', expected='unsafe'))
    ...
    t.add(Test('test-29-unsafe-2.c', 'test-29-unsafe-2.c', 'prover', 'error',
               expected='unsafe'))
    t.add(Test('test-30-safe.c', 'test-30-safe.c', 'prover', 'safe', expected='unsafe',
               line_checks=[(12, 'ok', 'warning'), (13, 'ok', 'warning')]))
    t.add(Test('test-30-unsafe.c', 'test-30-unsafe.c', 'prover', 'unsafe',
               line_checks=[(14, 'ok', 'warning'), (15, 'ok', 'warning'), (16, 'warning')]))
    t.add(Test('mine-lctes06.c', 'mine-lctes06.c', 'prover', 'safe',
               expected='unsafe', line_checks=[(30, 'ok'), (31, 'ok', 'warning')]))
    t.add(Test('astree.c', 'astree.c', 'prover', 'safe',
               expected='unsafe', line_checks=[(20, 'ok', 'warning'), (25, 'ok', 'warning')]))
    t.run()
