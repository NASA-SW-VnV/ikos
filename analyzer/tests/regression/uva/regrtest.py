...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Regression tests for the uninitialized variable analysis')
    ...
    t.add(Test('test-1-unsafe.c', 'test-1-unsafe.c', 'uva', 'error',
               line_checks=[(11, 'error')]))
    t.add(Test('test-2-safe.c', 'test-2-safe.c', 'uva', 'safe'))
    t.add(Test('test-2-unsafe.c', 'test-2-unsafe.c', 'uva', 'error',
               line_checks=[(6, 'error')]))
    t.add(Test('test-2-warning.c', 'test-2-warning.c', 'uva', 'unsafe',
               expected='safe'))
    t.add(Test('test-3-safe.c', 'test-3-safe.c', 'uva', 'safe', expected='unsafe'))
    t.add(Test('test-3-unsafe.c', 'test-3-unsafe.c', 'uva', 'error', expected='unsafe'))
    t.add(Test('test-4-unsafe.c', 'test-4-unsafe.c', 'uva', 'error',
               line_checks=[(4, 'error'),
                            (5, 'error'),
                            (11, 'error', 'warning'),
                            (19, 'error', 'warning')]))
    ...
    t.add(Test('test-8-unsafe.c', 'test-8-unsafe.c', 'uva', 'unsafe', expected='safe'))
    ...
    t.add(Test('test-13-unsafe.c', 'test-13-unsafe.c', 'uva', 'error', expected='unsafe'))
    t.add(Test('test-14-safe.c', 'test-14-safe.c', 'uva', 'safe'))
    t.add(Test('test-15-unsafe.c', 'test-15-unsafe.c', 'uva', 'error',
               line_checks=[(2, 'error')]))
    t.add(Test('test-16-unsafe.c', 'test-16-unsafe.c', 'uva', 'error',
               line_checks=[(2, 'error')]))
    t.add(Test('test-17-unsafe.c', 'test-17-unsafe.c', 'uva', 'error',
               line_checks=[(5, 'error')]))
    t.add(Test('test-18-unsafe.c', 'test-18-unsafe.c', 'uva', 'error',
               line_checks=[(5, 'error')]))
    t.add(Test('test-19-unsafe.c', 'test-19-unsafe.c', 'uva', 'error',
               line_checks=[(4, 'error')]))
    t.add(Test('test-20-safe.c', 'test-20-safe.c', 'uva', 'safe', expected='unsafe'))
    t.add(Test('test-20-safe.c', 'test-20-safe.c', 'uva', 'safe', options=['--ikos-pp']))
    t.add(Test('test-21-safe.cpp', 'test-21-safe.cpp', 'uva', 'safe'))
    t.add(Test('test-21-safe.cpp', 'test-21-safe.cpp', 'uva', 'safe', options=['--ikos-pp']))
    t.run()
