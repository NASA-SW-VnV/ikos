...
public:
  ...
  typedef varvec_map_t::const_iterator dead_const_iterator;

private:
  varset_map_t _live_map; //! live set
  varvec_map_t _dead_map; //! dead set
...
private:
  ...
  void storeLiveSet(Basic_Block_ref bb, liveness_domain_t inv) {
    unord_varname_set_t live;
    if (!inv.is_bottom()) {
      for (liveness_domain_t::iterator I = inv.begin(), E = inv.end(); I != E;
           ++I) {
        live.insert(*I);
      }
    }
  ...
  void storeDeadSet(Basic_Block_ref bb, liveness_domain_t inv) {
    std::vector< varname_t > dead;
    if (!inv.is_bottom()) {
      arbos_cfg::arbos_node_t node = this->get_cfg().get_node(bb);
      varname_set_t allVars = node.all_vars();
      for (varname_set_t::iterator I = allVars.begin(), E = allVars.end();
           I != E;
           ++I) {
        if (!(liveness_domain_t(*I) <= inv)) {
          dead.push_back(*I);
        }
      }
    }
    _dead_map.insert(varvec_map_t::value_type(bb, dead));
  }
...
public:
  ...
  boost::optional< unord_varname_set_t > liveSet(std::string F,
                                                 Basic_Block_ref bb) const {
    // We use boost::optional<unord_varname_set_t> () to express that
    // liveness analysis has not been enabled.

    const_iterator It = _map->find(F);
    if (It != _map->end()) {
      liveness_ptr L = It->second;
      Liveness::live_const_iterator BIt = L->_live_map.find(bb);
      if (BIt != L->_live_map.end()) {
        return boost::optional< unord_varname_set_t >(BIt->second);
      }
    }
    return boost::optional< unord_varname_set_t >();
  }

  //! return for a given function f and basic block bb the set of dead
  //! variables at the exit of bb.
  std::vector< varname_t > deadSet(std::string F, Basic_Block_ref bb) const {
    const_iterator It = _map->find(F);
    if (It != _map->end()) {
      liveness_ptr L = It->second;
      Liveness::dead_const_iterator BIt = L->_dead_map.find(bb);
      if (BIt != L->_dead_map.end()) {
        return BIt->second;
      }
    }
    return std::vector< varname_t >();
  }
