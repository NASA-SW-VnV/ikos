...
public:
  TransferFunctionVisitor(
      ...
      : _sym_exec(pre, vfac, lfac, prec_level, pointer_info, dead_vars),
...
// patricia_tree_set< variable >
patricia_tree_set< ikos::variable< number_t, varname_t > > from_internal_vars(
    const std::vector< Internal_Variable_ref >& internal_vars,
    LiteralFactory& lfac) {
  patricia_tree_set< ikos::variable< number_t, varname_t > > vars;

  for (typename std::vector< Internal_Variable_ref >::const_iterator it =
           internal_vars.begin();
       it != internal_vars.end();
       ++it) {
    vars += lfac[*it].get_var();
  }
...
// patricia_tree_set< variable >
patricia_tree_set< ikos::variable< number_t, varname_t > > from_local_vars(
  ...
  private:
    ...
    public:
      ...
      AbsNumDomain exec_call(Call_ref stmt, AbsNumDomain inv) {
        ...
        if (ar::isDirectCall(stmt)) {
          ...
          VariableName ptr = vfac[ar::getName(ar::getIndirectCallVar(stmt))];
    ...
    private:
      AbsNumDomain exec_call_with_summary(Call_ref stmt,
        ...
        sym_exec_t sym_exec(inv, vfac, lfac, PTR);
        ...
        sym_exec.match_down(formal_params, actual_params);
        ...
        // forget local vars, internal vars and formal parameters
        inv -= from_local_vars(cfg_fac[callee].get_local_variables(), lfac);
        inv -=
            from_internal_vars(cfg_fac[callee].get_internal_variables(), lfac);
      ...
      AbsNumDomain exec_call_forget(Call_ref stmt, AbsNumDomain inv) {
        if (ar::getReturnValue(stmt)) {
          VariableFactory& vfac = _summary_pass.var_factory();
          VariableName lhs = vfac[ar::getName(*(ar::getReturnValue(stmt)))];
          inv -= lhs;
        }
...
#ifdef DEBUG
      std::cout << "\t" << ar::getName(bb) << ": " << pre << std::endl;
#endif
...
private:
  typedef NumericalSummaryPass< SumAbsNumDomain, VariableName, Number >
  ...
  private:
    ...
    private:
      AbsNumDomain exec_match_parameters(Call_ref stmt,
        ...
        Function_ref caller = _numerical_analysis.get_cfg().get_func();

        num_sym_exec< AbsNumDomain, VariableName, Number >
            sym_exec(inv,
        ...
        // forget local vars and internal vars
        inv -= from_local_vars(cfg_fac[caller].get_local_variables(), lfac);
        inv -=
            from_internal_vars(cfg_fac[caller].get_internal_variables(), lfac);
      ...
      AbsNumDomain exec_call_with_summary(Call_ref stmt,
        ...
        num_sym_exec< SumAbsNumDomain, VariableName, Number > sym_exec(inv,
                                                                       vfac,
                                                                       lfac,
                                                                       PTR);
...
private:
  void save_pointer_info(const pta_system& csts_system, // solved constraints
    ...
    for (typename pt_var_map_t::const_iterator it = pt_var_map.begin();
      ...
      const pointer_var& pt_var = it->second;

      address_set points_to_addrs = csts_system.get(pt_var).first;
      z_interval offset = csts_system.get(pt_var).second;
      if (offset.is_bottom()) {
        offset = z_interval::top();
      }

      if (points_to_addrs.empty()) {
        _pointer_info.add(var_name,
                          std::make_pair(discrete_domain< VariableName >::top(),
                                         offset));
      } else {
        discrete_domain< VariableName > points_to_vars =
            discrete_domain< VariableName >::bottom();

        for (address_set::const_iterator a_it = points_to_addrs.begin();
  ...
  private:
    ...
    private:
      AbsValueDomain exec_call_with_summary(Call_ref stmt,
        ...
        sym_exec_t sym_exec(inv, vfac, lfac, MEM, _summary_pass.pointer_info());
        ...
        // forget local vars, internal vars and formal parameters
        inv.forget_mem_surface(
            from_local_vars(cfg_fac[callee].get_local_variables(), lfac));
        inv.forget_mem_surface(
            from_internal_vars(cfg_fac[callee].get_internal_variables(), lfac));
      ...
      AbsValueDomain exec_call_forget(Call_ref stmt, AbsValueDomain inv) {
        ...
        for (OpRange::iterator it = parameters.begin(); it != parameters.end();
             ++it) {
          if (ar::isPointer(*it)) {
            Literal param = _summary_pass.lit_factory()[*it];
            if (param.is_var()) {
              inv -= param.get_var();
            }
          }
...
private:
  ...
  typedef ValueSummaryPass< SumAbsValueDomain, VariableName, Number >
...
private:
  ...
  public:
    ...
    AbsValueDomain call(context& ctx,
                        Call_ref stmt,
                        AbsValueDomain inv,
  ...
  private:
    AbsValueDomain match_parameters(Call_ref stmt,
      ...
      num_sym_exec< AbsValueDomain, VariableName, Number >
          sym_exec(inv,
                   vfac,
                   lfac,
                   this->_prec_level,
      ...
      // assign parameters
      sym_exec.match_down(ar::getFormalParams(callee), ar::getArguments(stmt));

      inv = sym_exec.inv();

      // forget local vars and internal vars
      inv.forget_mem_surface(
          from_local_vars(cfg_fac[_function].get_local_variables(), lfac));
      inv.forget_mem_surface(
          from_internal_vars(cfg_fac[_function].get_internal_variables(),
                             lfac));
    ...
    AbsValueDomain call_with_summary(Call_ref stmt,
      ...
      num_sym_exec< SumAbsValueDomain, VariableName, Number >
          sym_exec(inv,
                   vfac,
                   lfac,
                   this->_prec_level,
    ...
    AbsValueDomain call_forget(Call_ref stmt, AbsValueDomain inv) {
      ...
      for (OpRange::iterator it = parameters.begin(); it != parameters.end();
           ++it) {
        if (ar::isPointer(*it)) {
          Literal param = _check_pass.lit_factory()[*it];
          if (param.is_var()) {
            inv -= param.get_var();
          }
        }
...
public:
...
#ifdef DEBUG
    std::cout << "call context  from " << path << ": " << inv << std::endl;
#endif
  ...
  void execute(Bundle_ref bundle,
    ...
    context ctx(db, bundle, empty_live, _pointer_info, _cfg_fac);
