...
inline GvRange getGlobalVars(const Bundle_ref& b) {
  ...
  // return boost::make_iterator_range (DEREF(b).getGlobalVariables ().begin (),
  //                                   DEREF(b).getGlobalVariables ().end ());
...
inline FuncRange getFunctions(const Bundle_ref& b) {
  assert(!ar_internal::is_null_ref(b));
  // return boost::make_iterator_range (DEREF(b).getFunctions ().begin (),
  //                                   DEREF(b).getFunctions ().end ());
...
inline IvRange getInternalVars(const Code_ref& c) {
  assert(!ar_internal::is_null_ref(c));
  // return boost::make_iterator_range (DEREF(c).getInternalVariables ().begin
  // (),
  //                                   DEREF(c).getInternalVariables ().end ());
  return DEREF(c).getInternalVariables();
}
//! blocks of the function
inline BBRange getBlocks(const Code_ref& c) {
  assert(!ar_internal::is_null_ref(c));
  // return boost::make_iterator_range (DEREF(c).getBlocks ().begin (),
  //                                   DEREF(c).getBlocks ().end ());
...
inline IvRange getFormalParams(const Function_ref& f) {
  assert(!ar_internal::is_null_ref(f));
  // return boost::make_iterator_range (DEREF(f).getFormalParams ().begin (),
  //                                   DEREF(f).getFormalParams ().end ());
...
inline BBRange getBlocks(const Function_ref& f) {
  ...
  assert(!ar_internal::is_null_ref(b));
  // return boost::make_iterator_range (DEREF(b).getBlocks ().begin (),
  //                                   DEREF(b).getBlocks ().end ());
...
inline StmtRange getStatements(const Basic_Block_ref& b) {
  assert(!ar_internal::is_null_ref(b));
  // return boost::make_iterator_range (DEREF(b).getStatements ().begin (),
  //                                   DEREF(b).getStatements ().end ());
  return DEREF(b).getStatements();
}
//! successor blocks
inline BBRange getSuccs(const Basic_Block_ref& b) {
  assert(!ar_internal::is_null_ref(b));
  // return boost::make_iterator_range (DEREF(b).getNextBlocks ().begin (),
  //                                   DEREF(b).getNextBlocks ().end ());
...
inline BBRange getPreds(const Basic_Block_ref& b) {
  assert(!ar_internal::is_null_ref(b));
  // return boost::make_iterator_range (DEREF(b).getPreviousBlocks ().begin (),
  //                                   DEREF(b).getPreviousBlocks ().end ());
...
inline OpRange getArguments(const Call_ref& cs) {
  ...
  // return boost::make_iterator_range (DEREF(cs).getArguments ().begin (),
  //                                   DEREF(cs).getArguments ().end ());
...
inline Type_ref getType(const Global_Variable_ref& gv) {
  assert(!ar_internal::is_null_ref(gv));
  Variable_ref v = node_cast< Variable >(gv);
  return DEREF(v).getType();
}
...
inline Type_ref getType(const Operand_ref& o) {
  ...
  if (ar_internal::is_var_operand(o)) {
    ...
    assert(ar_internal::is_cst_operand(o));
    ...
    assert(false && "Type of an operand cannot be determined");
    exit(EXIT_FAILURE);
  }
}
...
inline uint64_t getPointerSize(const Bundle_ref& b) {
  ...
  buf << sz;
  uint64_t r = boost::lexical_cast< uint64_t >(buf.str());
  return r;
}
...
inline std::string getZValueStr(const Integer_Constant_ref& n) {
  std::ostringstream buf;
  assert(!ar_internal::is_null_ref(n));
  buf << DEREF(n).getValue();
  return buf.str();
}
...
public:
  ...
  std::vector< Function_ref > pointedBy(
    ...
    VariableName v = _vfac[getName(internal_var)];
    ptr_set_t ptr_set = _ptr_info[v].first;
    std::vector< Function_ref > result;

    if (ptr_set.is_top()) {
      std::cout << "Warning: unable to solve indirect call on variable " << v
                << std::endl;
      return std::vector< Function_ref >();
    }

    for (ptr_set_t::iterator it = ptr_set.begin(); it != ptr_set.end(); ++it) {
      boost::optional< Function_ref > fun = getFunction(_bundle, (*it).name());
...
inline void convert(Internal_Variable_ref x, Operand_ref& y) {
  y = ARFactory::createVarOperand(x);
}
